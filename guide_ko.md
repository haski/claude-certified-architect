...
**Claude Certified Architect — Foundations** 인증은 실무 Claude 기반 솔루션을 구현할 때 전문가가 타당한 trade-off 결정을 내릴 수 있음을 확인합니다. 이 시험은 Claude로 프로덕션 애플리케이션을 구축하는 핵심 기술인 Claude Code, Claude Agent SDK, Claude API, Model Context Protocol(MCP)에 대한 기초 지식을 평가합니다.

시험 문제는 고객 지원용 Agentic 시스템 구축, multi-agent research pipeline 설계, Claude Code의 CI/CD 통합, 개발자 생산성 tool 생성, 비정형 문서에서 structured data extraction 등 현실적인 산업 scenario를 기반으로 합니다.

---

## 대상 응시자

이상적인 응시자는 Claude로 프로덕션 애플리케이션을 설계하고 출시하는 **솔루션 아키텍트**입니다. 다음 영역에서 최소 6개월 이상의 실무 경험이 있어야 합니다.
...
| 추측 페널티 | 없음(모든 문제에 답하세요!) |
| scenario | 가능한 8개 중 4개(무작위 선택) |
...
### scenario 1: 고객 지원 Agent
Claude Agent SDK를 사용해 반품, 청구 분쟁, 계정 문제를 처리하는 Agent를 구축합니다. Agent는 MCP tool(`get_customer`, `lookup_order`, `process_refund`, `escalate_to_human`)를 사용합니다. 목표는 적절한 escalation과 함께 80% 이상의 최초 접촉 해결률을 달성하는 것입니다.

### scenario 2: Claude Code를 사용한 코드 생성
...
### scenario 5: 지속적 통합을 위한 Claude Code
자동 코드 리뷰, 테스트 생성, 풀 리퀘스트 피드백을 위해 Claude Code를 CI/CD pipeline에 통합합니다. prompt는 false positive을 최소화하도록 설계해야 합니다.
...
### scenario 8: Agentic AI tool *(내용 누락 — 채워 넣을 수 있게 도와주세요!)*
이 scenario는 시험 응시자들이 보고했지만 아직 이 가이드에서 다루지 않습니다. 실제 시험에서 이 scenario의 문제를 접했다면 [GitHub Issues](https://github.com/paullarionov/claude-certified-architect/issues)에 공유해 주세요. 그러면 전체 범위를 추가할 수 있습니다. 여러분의 기여는 시험을 준비하는 모두에게 도움이 됩니다.
...
이 파트는 시험에 성공적으로 합격하는 데 필요한 모든 이론을 다룹니다. 자료는 시험 도메인이 아니라 기술과 개념을 기준으로 구성되어 있습니다. 이렇게 구성하면 각 주제를 더 깊이 이해하는 데 도움이 됩니다.
...
Claude API는 요청-응답 모델을 따릅니다. Claude Messages API에 대한 각 요청에는 다음이 포함됩니다.
...
`messages` 배열은 세 가지 역할을 사용합니다.
- `user` — 사용자 메시지
- `assistant` — 모델 응답(기록을 보낼 때 포함)
- `tool` — tool 호출 결과(역할이 명시적으로 설정되지는 않으며, `tool_result` 콘텐츠 블록으로 나타남)

**매우 중요:** 모든 API 요청에서 **전체 대화 기록**을 보내야 합니다. 모델은 요청 사이에 상태를 보존하지 않습니다. 각 호출은 독립적입니다.

## 1.3 응답의 `stop_reason` 필드

Claude API 응답에는 모델이 생성을 멈춘 이유를 나타내는 `stop_reason`이 포함됩니다.
...
Agentic 시스템에서는 `"tool_use"`와 `"end_turn"`이 가장 중요합니다. 이 둘이 Agent 루프를 제어합니다.

## 1.4 system prompt

system prompt는 context와 동작 규칙을 정의하는 특수 지시사항입니다. system prompt는 다음과 같습니다.
- `messages` 배열의 일부가 아니며 `system` 필드에 별도로 전달됨
- 사용자 메시지보다 우선순위가 높음
- 한 번 로드되어 대화 전체에 적용됨
- 역할, 제약, 출력 형식을 정의하는 데 사용됨

**시험에서 중요한 점:** system prompt 문구는 의도치 않은 tool 연관을 만들 수 있습니다. 예를 들어 "항상 고객을 확인하라" 같은 지시사항은 필요하지 않은 경우에도 모델이 `get_customer`를 과도하게 사용하게 만들 수 있습니다.

## 1.5 context window

context window는 모델이 한 번에 처리할 수 있는 텍스트의 총량(token 기준)입니다. 여기에는 다음이 포함됩니다.
- system prompt
- 전체 메시지 기록
- tool 정의
- tool 결과
...
1. **중간 손실 효과:** 모델은 긴 입력의 시작과 끝에 있는 정보를 안정적으로 처리하지만 중간의 세부사항을 놓칠 수 있습니다. 완화 방법: 핵심 정보를 시작이나 끝 근처에 batch합니다.

2. **tool 결과 누적:** 모든 tool 호출은 context에 출력을 추가합니다. tool이 40개 이상의 필드를 반환하지만 실제로 중요한 필드가 5개뿐이라면 대부분의 context가 낭비됩니다.

3. **점진적 summary:** 기록을 압축할 때 숫자 값, 백분율, 날짜가 자주 손실되어 "대략", "거의", "몇 개"처럼 모호해집니다.
...
`tool_use`는 Claude가 외부 함수를 호출할 수 있게 하는 메커니즘입니다. 모델은 코드를 직접 실행하지 않습니다. 구조화된 tool 호출 요청을 생성하고, 사용자의 코드가 이를 실행한 뒤 결과를 반환합니다.
...
1. **설명은 기본 선택 메커니즘입니다.** LLM은 설명을 기준으로 tool을 선택합니다. 최소한의 설명("고객 정보를 검색함")은 tool이 겹칠 때 실수를 유발합니다.
...
3. tool 간에 동일하거나 겹치는 설명은 **피하세요**. `analyze_content`와 `analyze_document`의 설명이 거의 동일하면 모델은 두 tool을 혼동합니다.

4. **built-in tool vs MCP tool:** Agent는 유사한 기능의 MCP tool보다 built-in tool(Read, Grep)를 선호할 수 있습니다. 이를 방지하려면 MCP tool 설명을 강화하세요. built-in tool이 제공할 수 없는 구체적인 장점, 고유 데이터, context를 강조합니다.

## 2.3 `tool_choice` 매개변수

`tool_choice`는 모델이 tool을 선택하는 방식을 제어합니다.

| 값 | 동작 | 사용 시점 |
|---|---|---|
| `{"type": "auto"}` | 모델이 tool을 호출할지 텍스트로 답할지 결정 | 대부분의 경우 기본값 |
| `{"type": "any"}` | 모델이 **반드시** 어떤 tool든 호출해야 함 | structured output을 반드시 받아야 할 때 |
| `{"type": "tool", "name": "extract_metadata"}` | 모델이 **반드시** 특정 tool을 호출해야 함 | 강제 첫 단계나 실행 순서가 필요할 때 |
...
JSON schema와 함께 `tool_use`를 사용하는 것은 Claude에서 structured output을 얻는 **가장 신뢰할 수 있는** 방법입니다. 이는 다음을 제공합니다.
- 구문상 유효한 JSON 보장(누락된 중괄호나 trailing comma 없음)
- 필요한 구조 강제(필수 필드 존재)
- 의미적 정확성은 보장하지 않음(값은 여전히 틀릴 수 있음)
...
**schema 설계 규칙:**
1. **필수 vs 선택:** 정보가 항상 사용 가능한 경우에만 필드를 필수로 표시합니다. 데이터가 없을 때 필수 필드는 모델이 값을 지어내도록 압박합니다.
2. **Nullable 필드:** 없을 수 있는 정보에는 `"type": ["string", "null"]`을 사용합니다. 모델은 환각 대신 `null`을 반환할 수 있습니다.
3. **`"other"`가 있는 enum:** 미리 정의한 범주 밖의 데이터를 잃지 않도록 `"other"`와 세부 문자열을 추가합니다.
4. **Enum `"unclear"`:** 모델이 확신 있게 범주를 고를 수 없는 경우에 사용합니다. 정직한 `"unclear"`가 잘못된 범주보다 낫습니다.
...
Agentic 루프는 자율 작업 실행을 위한 핵심 패턴입니다. 모델은 단순히 답변만 하지 않고 일련의 동작을 수행합니다.
...
**이는 모델 주도 접근 방식입니다:** Claude는 context와 이전 tool 결과를 기준으로 다음에 호출할 tool을 결정합니다. 이는 동작 순서가 고정된 하드코딩 의사결정 트리와 다릅니다.
...
multi-agent architecture는 일반적으로 허브 앤 스포크 토폴로지로 구축됩니다.
...
**병렬 생성:** Coordinator는 한 응답에서 여러 `Task`를 호출할 수 있으며, subagent는 병렬로 실행됩니다.
...
hook은 Agent 수명 주기의 특정 지점에서 가로채기와 변환을 가능하게 합니다.
...
Model Context Protocol(MCP)은 외부 시스템을 Claude에 연결하기 위한 개방형 프로토콜입니다. MCP는 세 가지 기본 resource 유형을 정의합니다.
...
MCP 서버는 MCP 프로토콜을 구현하고 tool/resource를 제공하는 프로세스입니다. MCP 서버에 연결하면 다음이 발생합니다.
- 모든 tool이 자동으로 발견됨
...
MCP tool에서 오류가 발생하면 응답에 `isError: true`를 사용합니다. 이는 호출이 실패했음을 Agent에 알립니다.
...
일반 오류는 Agent가 의사결정할 정보를 제공하지 않습니다. retry해야 할까요, 쿼리를 바꿔야 할까요, 아니면 escalation해야 할까요?

## 4.5 MCP resource

resource는 Agent가 동작을 수행하지 않고 context를 얻기 위해 요청할 수 있는 데이터입니다.
...
**resource의 장점:** Agent가 어떤 데이터가 존재하는지 이해하기 위해 탐색적 tool 호출을 할 필요가 없습니다. resource는 즉시 사용할 수 있는 "지도"를 제공합니다.
...
CLAUDE.md는 Claude Code를 위한 지시사항 파일입니다. 3단계 계층이 있습니다.
...
**일반적인 실수:** 새 팀원이 프로젝트 지시사항을 받지 못하는 경우가 있습니다. 지시사항이 `.claude/CLAUDE.md`(프로젝트 수준)가 아니라 `~/.claude/CLAUDE.md`(사용자 수준)에 놓였기 때문입니다.

## 5.2 `@path` 문법(파일 가져오기)

CLAUDE.md는 `@path`를 사용해 외부 파일을 참조할 수 있으며, 이를 통해 구성을 module화할 수 있습니다.
...
이는 중복을 피하고 각 패키지가 관련 표준만 포함하게 해줍니다.

## 5.3 `.claude/rules/` 디렉터리

`.claude/rules/`는 단일 CLAUDE.md의 대안이며, 주제별로 규칙을 구성하는 데 사용됩니다.
...
**작동 방식:**
- Claude Code가 `paths` 패턴과 일치하는 파일을 편집할 때만 규칙이 **로드됨**
- 관련 없는 규칙이 로드되지 않으므로 context와 token을 절약함
- Glob 패턴을 사용하면 위치와 관계없이 파일 유형별 규칙을 적용할 수 있음(코드베이스 전체에 흩어진 테스트에 이상적)
...
슬래시 명령은 `/name`으로 호출되는 재사용 가능한 prompt template입니다.
...
Skill은 SKILL.md frontmatter로 구성되는 고급 명령입니다.
...
| 매개변수 | 설명 |
|---|---|
| `context: fork` | 격리된 subagent에서 Skill을 실행합니다. 상세 출력이 메인 session을 오염시키지 않습니다 |
...
**계획 모드:**
- 모델은 조사와 계획만 수행하며 변경하지 않음
- Read, Grep, Glob을 사용해 코드베이스를 탐색
- 사용자가 승인할 구현 계획을 생성
- 부작용 없는 안전한 탐색
...
- 메인 context에서 상세 출력을 격리
- summary만 반환
- 다단계 작업에서 context window 고갈 방지

## 5.7 `/compact` 명령

`/compact`는 context를 압축하는 내장 명령입니다.
- 이전 기록을 summary해 context window를 확보함
- 긴 조사 session에서 상세 tool 출력으로 context가 가득 찰 때 사용
- 위험: summary 중 정확한 숫자 값, 날짜, 구체적 세부사항이 손실될 수 있음

## 5.8 `/memory` 명령

`/memory`는 session 간 memory를 관리하는 내장 명령입니다.
- `CLAUDE.md` 파일을 편집용으로 열어 노트, 선호사항, context를 저장할 수 있게 함
- 정보는 session 간 유지되며 시작 시 자동 로드됨
- 프로젝트 규칙, 사용자 선호사항, 자주 쓰는 명령, 현재 작업 context 저장에 유용함
- 매 session 같은 지시사항을 다시 설명하는 것의 대안
...
**session context 격리:**
코드를 생성한 동일 Claude session은 그 코드를 리뷰할 때 효과가 떨어지는 경우가 많습니다(모델이 자신의 추론 context를 유지하므로 스스로의 결정을 덜 비판할 가능성이 큼). 리뷰에는 독립 instance를 사용하세요.
...
Few-shot prompting은 기대 동작을 보여주기 위해 prompt에 2-4개의 입력/출력 예시를 포함하는 방식입니다.

**Few-shot이 텍스트 설명보다 효과적인 이유:**
- "더 정확하게 하라" 같은 모호한 지시는 여러 방식으로 해석될 수 있음
- 예시는 기대 형식과 판단 로직을 명확하게 보여줌
- 모델은 패턴을 새 사례에 일반화함(예시를 단순 반복하는 것이 아님)

**Few-shot 예시 유형과 사용 시점:**

1. **모호한 scenario를 위한 예시:**
...
Few-shot은 순수 규칙 기반 지시사항으로 처리하기에는 너무 다양한 비공식적이고 비표준적인 측정 단위를 extraction할 때 특히 효과적입니다.
...
이는 JSON은 구문상 유효하지만 값이 일관되지 않은 의미 오류를 방지합니다.
...
prompt chaining은 복잡한 작업을 집중된 단계의 순서로 나눕니다.
...
**중요한 이유:**
- **주의 분산**을 피함. 모델이 너무 많은 파일을 한 번에 받으면 일부 파일의 버그를 놓치면서 다른 파일에는 얕은 의견만 제공할 수 있음
- 파일별 분석 품질을 일관되게 보장함
- 파일 간 상호작용을 별도로 분석할 수 있음
...
솔루션을 구현하기 전에 Claude가 clarification 질문을 합니다.
...
**유용한 경우:**
- 낯선 도메인(핀테크, 헬스케어, 법률 시스템)
- 함의가 명확하지 않은 작업(cache 전략, 실패 모드)
- 여러 접근 방식이 가능하고 최선의 선택이 context에 의존하는 경우

## 6.5 validation과 피드백 기반 retry

extraction된 데이터가 validation에 실패했을 때:
...
**retry가 효과적인 경우:**
- 형식 오류(잘못된 형식의 날짜)
- 구조 오류(필드가 잘못된 위치에 batch됨)
- 산술 불일치(모델이 재확인할 수 있음)
...
모델은 명시된 값과 계산된 값을 모두 extraction합니다. 두 값이 다르면 `conflict_detected`를 통해 불일치를 처리할 수 있습니다.
...
Message Batches API를 사용하면 비synchronous 처리를 위해 요청 batch를 제출할 수 있습니다.

| 속성 | 값 |
|---|---|
| 비용 절감 | synchronous 호출 대비 **50%** |
| 처리 window | 최대 **24시간**(지연 시간 SLA 보장 없음) |
| 멀티턴 tool 호출 | **지원하지 않음**(요청 1개 = 응답 1개) |
| 상관관계 | 요청과 응답을 연결하는 `custom_id` 필드 |

## 7.2 Batch API와 synchronous API 사용 기준

| 작업 | API | 이유 |
|---|---|---|
| 병합 전 PR 검사 | **synchronous** | 개발자가 기다리고 있으며 24시간은 허용 불가 |
| 야간 기술 부채 보고서 | **Batch** | 아침까지 결과가 필요하며 50% 비용 절감 |
| 주간 보안 감사 | **Batch** | 긴급하지 않으며 50% 비용 절감 |
| 대화형 코드 리뷰 | **synchronous** | 즉각 응답 필요 |
...
30시간 안에 결과가 필요하고 Batch API가 최대 24시간 걸릴 수 있다면:
- 제출 window: 30 - 24 = **6시간**
- batch는 마감 최소 24시간 전에 제출되어야 함
- 자주 제출하는 경우 4시간 window로 분할
...
**14개 파일을 한 번에 단일 패스로 보는 것이 나쁜 이유:**
- 주의 분산: 일부 파일은 깊이 분석하고 다른 파일은 얕게 분석함
- 일관되지 않은 댓글: 한 파일에서는 패턴을 문제로 표시하지만 다른 파일에서는 승인함
- 놓친 버그: 인지 부하 때문에 명백한 오류를 건너뜀
...
핵심 원칙: 먼저 감정을 인정하고, 그다음 구체적 해결책을 제안하며, 고객이 사람과 이야기하고 싶다는 의사를 반복할 때만 escalation합니다. 불만의 첫 표현만으로 escalation하지 마세요. 이는 관리자 요청과 같지 않습니다.
...
사람 운영자는 전체 대화 전문에 접근할 수 없고 이 summary만 봅니다. 따라서 summary는 완전하고 자체적으로 이해 가능해야 합니다.

## 9.4 confidence calibration과 사람의 감독

데이터 extraction 시스템의 경우:

1. **필드 수준 confidence 점수:** 모델이 extraction 필드별 confidence 점수를 출력
...
이는 Coordinator가 결정하는 데 필요한 정보를 제공합니다.
- 수정된 쿼리로 retry할 것인가?
- 부분 결과를 사용할 것인가?
...
대화 기록(summary 중 저하될 수 있음)에 의존하는 대신 핵심 사실을 structured 블록으로 extraction합니다.
...
이는 context를 절약하고 노이즈를 줄입니다.

## 11.3 위치 인식 입력

중간 손실 효과를 고려해 중요한 정보를 batch합니다.
...
context가 저하되거나 새 session에서 Agent는 탐색을 다시 실행하는 대신 스크래치패드를 참조할 수 있습니다.
...
**분리된 context 계층:**
multi-agent 시스템에서 각 subagent는 제한된 context 예산 안에서 동작합니다. 즉, 자신의 작업에 필요한 정보만 받습니다. Coordinator는 별도 context 계층으로 작동합니다. subagent 출력을 집계하고, 전역 상태를 저장하며, context를 할당합니다. 이를 통해 한 Agent가 다른 Agent와 무관한 정보로 window를 소비하는 "context 누수"를 방지합니다.
...
임의로 한 값을 선택하지 마세요. 두 값 모두 source와 함께 보존하고 Coordinator가 결정하게 하세요.
...
- tool 결과는 대화 기록에 추가되어 모델이 다음 동작을 결정할 수 있게 함
...
### 핵심 기술:
- 이전 단계가 완료될 때까지 downstream 호출을 차단하는 프로그래밍적 사전조건(예: `get_customer`가 validated ID를 반환할 때까지 `process_refund` 차단)
- 여러 측면을 가진 고객 요청을 별도 항목으로 분해
- 사람에게 escalation할 때 structured summary 생성
...
- 오래된 결과로 재개하는 것보다 structured summary으로 새 session을 시작하는 편이 더 신뢰할 수 있음
...
### 핵심 기술:
- `errorCategory`(transient/validation/permission), `isRetryable`, 사람이 읽을 수 있는 메시지 같은 structured metadata 반환
...
- `context: fork`는 Skill을 격리된 subagent context에서 실행해 메인 session을 오염시키지 않음
...
- 계획 모드는 변경 전에 코드베이스를 안전하게 탐색할 수 있게 함
...
- false positive이 개발자 신뢰에 미치는 영향: 일부 범주의 높은 false positive률은 정확한 범주에 대한 confidence 약화시킴

### 핵심 기술:
- 리뷰 기준 정의: 보고할 것(버그, 보안) vs 무시할 것(사소한 스타일)
- false positive률이 높은 범주를 일시적으로 비활성화
...
### 핵심 기술:
- 근거와 함께 모호한 scenario를 위한 목표 예시 2-4개 제공
...
### 핵심 기술:
- 차단 검사는 synchronous API 사용, 야간/주간 워크로드는 Batch API 사용
...
### 핵심 지식:
- 점진적 summary의 위험: 숫자 값, 백분율, 날짜가 모호한 summary으로 압축됨
...
### 핵심 지식:
- structured error context(실패 유형, 쿼리, 부분 결과, 대안)는 Coordinator의 더 나은 복구를 가능하게 함
...
- 종합 결과에 커버리지 주석 추가: 잘 뒷받침되는 부분과 공백이 남은 부분
...
- 구조화된 상태 지속성은 크래시 복구를 가능하게 함

### 핵심 기술:
- 메인 Agent에서 고수준 조정을 유지하면서 구체적 질문에는 subagent 생성
- 핵심 발견을 저장하고 나중에 참조하기 위해 스크래치패드 파일 사용
- 다음 단계 subagent를 생성하기 전에 핵심 발견 summary
- 긴 조사 중 context 사용량을 줄이기 위해 `/compact` 사용

## 5.5 사람 감독과 confidence calibration을 갖춘 workflow 설계
...
- 안정적 성능 validation을 위해 문서 유형과 필드별 accuracy 분석
- 필드 수준 confidence 점수를 출력하고 라벨링 데이터로 리뷰 임계값 calibration
...
- 안정적인 발견과 논쟁적 발견을 분리하도록 보고서 structured
...
## 문제 1(Scenario: 고객 지원 Agent)

**상황:** 데이터에 따르면 12%의 경우 Agent가 `get_customer`를 건너뛰고 고객 이름만으로 `lookup_order`를 호출하여 잘못된 환불이 발생합니다.

**가장 효과적인 변경은 무엇인가요?**

- A) `get_customer`에서 ID를 얻을 때까지 `lookup_order`와 `process_refund`를 차단하는 프로그래밍적 사전조건 추가 **[정답]**
- B) system prompt 개선
- C) Few-shot 예시 추가
...
**B인 이유:** tool 설명은 모델의 기본 선택 메커니즘입니다. 이는 가장 적은 노력으로 가장 큰 효과를 내는 수정입니다. A는 근본 원인을 다루지 않고 token만 추가합니다. C는 과도한 엔지니어링입니다. D는 정당화하기 어려울 만큼 더 많은 노력이 필요합니다.

---

## 문제 3(Scenario: 고객 지원 Agent)
...
**A인 이유:** 근본 원인인 불명확한 결정 경계를 직접 다룹니다. B는 신뢰할 수 없습니다(모델은 확신 있게 틀릴 수 있음). C는 과도한 엔지니어링입니다. D는 다른 문제를 풉니다(기분 != 복잡도).

---

## 문제 4(Scenario: Claude Code를 사용한 코드 생성)
...
## 문제 5(Scenario: Claude Code를 사용한 코드 생성)
...
**A인 이유:** 계획 모드는 큰 변경, 여러 가능한 접근, architecture 결정에 맞게 설계되었습니다. B는 비용 큰 재작업 위험이 있습니다. C는 이미 구조를 알고 있다고 가정합니다. D는 사후 대응적입니다.

---

## 문제 6(Scenario: Claude Code를 사용한 코드 생성)
...
**A인 이유:** glob 패턴(예: `**/*.test.tsx`)이 있는 `.claude/rules/`는 파일 경로를 기반으로 규칙을 자동 적용할 수 있어 코드베이스 전반에 흩어진 테스트에 이상적입니다. B는 모델 추론에 의존합니다. C는 수동/on-demand입니다. D는 관련 파일이 여러 디렉터리에 있을 때 잘 작동하지 않습니다.

---

## 문제 7(Scenario: Multi-Agent Research System)
...
**B인 이유:** 로그는 Coordinator가 "창작 산업"을 시각 하위 주제로만 분해해 음악, 문학, 영화를 완전히 놓쳤음을 보여줍니다. subagent는 올바르게 실행했습니다. 문제는 할당된 작업입니다.

---

## 문제 8(Scenario: Multi-Agent Research System)

**상황:** 웹 검색 subagent가 복잡한 주제를 조사하다 타임아웃됩니다. 오류 정보가 Coordinator로 전달되는 방식을 설계해야 합니다.

**지능적 복구를 가장 잘 가능하게 하는 오류 전파 접근 방식은 무엇인가요?**
...
**A인 이유:** structured error context는 Coordinator가 수정된 쿼리로 retry할지, 대안 접근을 시도할지, 부분 결과로 계속할지 결정하는 데 필요한 정보를 제공합니다. B는 일반 상태 뒤에 context를 숨깁니다. C는 실패를 성공으로 위장합니다. D는 전체 workflow를 불필요하게 중단합니다.

---

## 문제 9(Scenario: Multi-Agent Research System)

**상황:** 종합 Agent는 결과를 병합하면서 특정 주장을 validation해야 하는 경우가 많습니다. 현재 validation이 필요하면 종합 Agent가 제어를 Coordinator에게 돌려주고, Coordinator가 웹 검색 Agent를 호출한 뒤 새 결과로 종합을 다시 실행합니다. 이로 인해 작업당 왕복이 2-3회 추가되고 지연 시간이 40% 증가합니다. 평가 결과 이러한 확인의 85%는 단순 사실 확인(날짜, 이름, 통계)이고 15%는 더 깊은 조사가 필요합니다.

**안정성을 유지하면서 overhead를 줄이려면 어떻게 해야 하나요?**

- A) 종합 Agent에 단순 확citation 제한된 `verify_fact` tool을 제공하고, 복잡한 validation은 계속 Coordinator를 통해 routing **[정답]**
...
**A인 이유:** 이는 최소 권한 원칙을 적용합니다. 종합 Agent는 85%의 일반 사례(단순 사실 확인)에 필요한 것만 받고, 복잡한 조사를 위해서는 Coordinator 경유 경로를 유지합니다. B는 차단 의존성을 도입합니다(나중 종합 단계가 앞서 validated 사실에 의존할 수 있음). C는 책임 분리를 깨뜨립니다. D는 필요를 안정적으로 예측할 수 없는 추측성 caching에 의존합니다.

---

## 문제 10(Scenario: CI를 위한 Claude Code)
...
**A인 이유:** `-p`(또는 `--print`)는 Claude Code를 비대화형 모드로 실행하는 문서화된 방법입니다. prompt를 처리하고 stdout에 출력한 뒤 종료합니다. 다른 옵션은 존재하지 않는 기능이거나 Unix 우회 방법입니다.

---

## 문제 11(Scenario: CI를 위한 Claude Code)
...
**A인 이유:** Message Batches API는 50%를 절감하지만 처리 시간이 최대 24시간이고 지연 시간 SLA가 보장되지 않습니다. 따라서 개발자가 기다리는 차단형 병합 전 검사에는 부적합하지만, 기술 부채 보고서 같은 야간 batch 워크로드에는 이상적입니다.

---

## 문제 12(Scenario: 다중 파일 코드 리뷰)
...
**A인 이유:** 집중 패스는 많은 파일을 한 번에 처리할 때 발생하는 주의 분산이라는 근본 원인을 직접 다룹니다. 파일별 분석은 일관된 깊이를 보장하고, 별도 통합 패스는 파일 간 이슈를 잡습니다. B는 시스템 개선 없이 부담을 개발자에게 전가합니다. C는 오해입니다. 더 큰 context는 주의 품질을 고치지 않습니다. D는 일관되지 않은 탐지 간 합의를 요구해 실제 버그를 억제합니다.

---

# 연습 테스트

> 4개 scenario에 걸친 60개 문제입니다. 형식과 난이도는 실제 시험과 일치합니다.
> 
> 또는 시험과 유사한 HTML 파일에서 이 문제들을 연습할 수 있습니다: [Practical Test (EN)](practical_test_en.html)

## Scenario: Multi-Agent Research System

---

## 문제 1(Scenario: Multi-Agent Research System)

**상황:** 문서 분석 Agent가 핵심 지표에 대해 두 개의 신뢰할 수 있는 source가 직접 모순되는 통계를 포함하고 있음을 발견합니다. 정부 보고서는 40% 성장을 말하고, 산업 분석은 12%를 말합니다. 두 source 모두 신뢰할 만해 보이며, 이 불일치는 research 결론에 중대한 영향을 줄 수 있습니다. 문서 분석 Agent는 이 상황을 가장 효과적으로 어떻게 처리해야 하나요?
...
- D) 두 숫자로 분석을 완료하되 source attribution과 함께 충돌을 명시적으로 주석 처리하고, 종합으로 넘기기 전에 Coordinator가 데이터를 조정할 방법을 결정하게 한다. **[정답]**

**D인 이유:** 이 접근은 책임 분리를 보존합니다. 분석 Agent는 차단 없이 핵심 작업을 완료하고, 충돌하는 두 값을 명확한 attribution과 함께 보존하며, 더 넓은 context를 가진 Coordinator에게 조정을 올바르게 전달합니다.

---

## 문제 2(Scenario: Multi-Agent Research System)
...
**C인 이유:** Coordinator-subagent architecture에서 Coordinator는 두 결과 집합을 중앙 통합을 위한 종합 Agent에게 전달하여 제어를 보존하고 고품질 병합을 보장합니다.

---

## 문제 3(Scenario: Multi-Agent Research System)
...
## 문제 4(Scenario: Multi-Agent Research System)

**상황:** "AI가 창작 산업에 미치는 영향"에 대해 시스템을 실행한 뒤, 모든 subagent가 성공적으로 완료되는 것을 확인했습니다. 웹 검색 Agent는 관련 기사를 찾고, 문서 분석 Agent는 이를 올바르게 summary하며, 종합 Agent는 일관된 텍스트를 생성합니다. 하지만 최종 보고서는 시각 예술만 다루고 음악, 문학, 영화를 완전히 놓칩니다. Coordinator 로그에는 주제가 "디지털 아트의 AI", "그래픽 디자인의 AI", "사진의 AI"라는 세 하위 작업으로 분해된 것이 보입니다. 가장 가능성 높은 근본 원인은 무엇인가요?

**가장 가능성 높은 근본 원인은 무엇인가요?**
...
**C인 이유:** Coordinator가 넓은 주제를 시각 예술 하위 작업으로만 분해해 음악, 문학, 영화를 완전히 놓쳤습니다. subagent는 할당된 작업을 올바르게 실행했으므로, 좁은 분해가 명백한 근본 원인입니다.

---

## 문제 5(Scenario: Multi-Agent Research System)
...
- D) 사용할 수 없는 source로 인한 공백과 잘 뒷받침되는 결론을 나타내는 커버리지 주석으로 종합 출력을 구조화한다. **[정답]**

**D인 이유:** 커버리지 주석은 투명성을 갖춘 우아한 성능 저하를 구현합니다. 완료된 작업의 가치를 보존하면서 불확실성을 전파해 confidence에 대한 정보 기반 결정을 가능하게 합니다.

---

## 문제 6(Scenario: Multi-Agent Research System)

**상황:** 문서 분석 subagent가 parsing할 수 없는 손상된 PDF 파일을 만났습니다. 시스템 오류 처리를 설계할 때 이 실패를 처리하는 가장 효과적인 방법은 무엇인가요?

**가장 효과적인 접근 방식은 무엇인가요?**

- A) Coordinator Agent에게 context가 포함된 오류를 반환해 진행 방법을 결정할 수 있게 한다. **[정답]**
- B) workflow 중단을 피하기 위해 손상된 문서를 조용히 건너뛰고 나머지 파일 처리를 계속한다.
- C) 실패를 보고하기 전에 지수 백오프로 문서 parsing을 세 번 자동 retry한다.
- D) 전체 research workflow를 종료하는 예외를 던진다.

**A인 이유:** context가 포함된 오류를 Coordinator에게 반환하는 것이 가장 효과적입니다. Coordinator가 파일을 건너뛸지, 대체 parsing 방법을 시도할지, 사용자에게 알릴지 정보에 기반해 결정할 수 있으며 실패에 대한 가시성을 유지합니다.
...
**B인 이유:** 웹 검색 tool 이름을 `extract_web_results`로 바꾸고 설명에 웹 검색과 URL을 명시하면 두 tool 이름과 설명 사이의 의미적 겹침을 제거해 근본 원인을 직접 해결합니다. 각 tool 목적이 명확해져 Coordinator가 문서 분석과 웹 검색을 안정적으로 구분할 수 있습니다.

---

## 문제 8(Scenario: Multi-Agent Research System)

**상황:** 동료가 문서 분석 Agent가 Coordinator를 우회해 종합 Agent에게 결과를 직접 보내야 한다고 제안합니다. subagent 간 모든 통신에서 Coordinator를 중앙 허브로 유지하는 주요 장점은 무엇인가요?

**Coordinator를 중앙 허브로 유지하는 주요 장점은 무엇인가요?**
...
- C) Coordinator를 통한 routing은 직접 Agent 간 호출이 지원할 수 없는 자동 retry 로직을 가능하게 한다.
- D) subagent는 격리된 memory를 사용하며, 직접 통신은 Coordinator만 수행할 수 있는 복잡한 직렬화를 요구한다.

**A인 이유:** Coordinator 패턴은 모든 상호작용에 대한 중앙 가시성, 시스템 전반의 일관된 오류 처리, 각 subagent가 받는 정보에 대한 세밀한 제어를 제공합니다. 이것이 별 모양 통신 토폴로지의 주요 장점입니다.

---

## 문제 9(Scenario: Multi-Agent Research System)

**상황:** 웹 검색 subagent가 복잡한 주제를 조사하다 타임아웃됩니다. 이 실패에 대한 정보가 Coordinator에게 반환되는 방식을 설계해야 합니다. 지능적 복구를 가장 잘 가능하게 하는 오류 전파 접근 방식은 무엇인가요?

**지능적 복구를 가장 잘 가능하게 하는 오류 전파 접근 방식은 무엇인가요?**

- A) 실패 유형, 실행된 쿼리, 부분 결과, 잠재적 대안 접근을 포함한 structured error context를 Coordinator에게 반환한다. **[정답]**
...
**A인 이유:** 실패 유형, 실행된 쿼리, 부분 결과, 대안 접근을 포함한 structured error context를 반환하면 Coordinator가 지능적 복구 결정(예: 수정된 쿼리로 retry 또는 부분 결과로 계속)을 내리는 데 필요한 모든 것을 얻습니다. 이는 조정 수준의 정보 기반 의사결정을 위해 최대한의 context를 보존합니다.

---

## 문제 10(Scenario: Multi-Agent Research System)

**상황:** 시스템 설계에서 문서 분석 Agent가 URL로 문서를 다운로드할 수 있도록 범용 tool `fetch_url` 접근 권한을 부여했습니다. 프로덕션 로그는 이 Agent가 이제 임시 웹 검색을 수행하기 위해 검색 엔진 결과 페이지를 자주 다운로드한다는 것을 보여줍니다. 이 동작은 웹 검색 Agent를 통해 routing되어야 하며, 현재는 결과 불일치를 유발합니다. 가장 효과적인 수정은 무엇인가요?
...
**A인 이유:** 범용 tool을 문서 형식에 대해 URL을 validation하는 문서 전용 tool로 교체하면 interface 수준에서 기능을 제한해 근본 원인을 해결합니다. 이는 최소 권한 원칙을 따르며, 원치 않는 검색 동작을 단순히 discouraging하는 대신 불가능하게 만듭니다.

---

## 문제 11(Scenario: Multi-Agent Research System)

**상황:** 넓은 주제를 조사하는 동안 웹 검색 Agent와 문서 분석 Agent가 같은 하위 주제를 조사해 출력에 상당한 중복이 발생합니다. token 사용량은 거의 두 배가 되지만 research 폭이나 깊이는 비례해 증가하지 않습니다. 이를 해결하는 가장 효과적인 방법은 무엇인가요?

**이를 해결하는 가장 효과적인 방법은 무엇인가요?**
...
**B인 이유:** Coordinator가 위임 전에 research 공간을 명시적으로 분할하는 것이 가장 효과적입니다. 작업이 시작되기 전에 불명확한 작업 경계라는 근본 원인을 다루기 때문입니다. 병렬성을 보존하면서 중복 작업과 token 낭비를 방지합니다.

---

## 문제 12(Scenario: Multi-Agent Research System)

**상황:** research 중 웹 검색 subagent가 세 source 범주를 조회했으며 결과가 서로 다릅니다. 학술 데이터베이스는 관련 논문 15편을 반환하고, 산업 보고서는 "0 results"를 반환하며, 특허 데이터베이스는 "Connection timeout"을 반환합니다. Coordinator로의 오류 전파를 설계할 때 어떤 접근 방식이 가장 좋은 복구 결정을 가능하게 하나요?

**어떤 접근 방식이 가장 좋은 복구 결정을 가능하게 하나요?**

- A) 결과를 단일 성공 비율 지표(예: "67% source coverage")로 집계하고 자세한 로그는 요청 시 제공한다.
- B) "timeout"과 "0 results"를 모두 Coordinator 개입이 필요한 실패로 보고한다.
- C) 일시적 실패를 내부적으로 retry하고 지속적인 오류만 보고한다.
- D) retry 결정이 필요한 접근 실패(timeout)와 성공한 쿼리를 나타내는 유효한 빈 결과("0 results")를 구분한다. **[정답]**

**D인 이유:** 타임아웃(접근 실패)과 "0 results"(유효한 빈 결과)는 의미적으로 다른 결과이며 서로 다른 대응이 필요합니다. 이를 구분하면 Coordinator가 특허 데이터베이스는 retry하고, 산업 보고서의 "0 results"는 유효하고 정보성 있는 발견으로 받아들일 수 있습니다.

---

## 문제 13(Scenario: Multi-Agent Research System)

**상황:** 프로덕션 모니터링에서 종합 품질이 일관되지 않음을 확인했습니다. 집계 결과가 약 75K token일 때, 종합 Agent는 첫 15K token(웹 검색 헤드라인/스니펫)과 마지막 10K token(문서 분석 결론)의 정보는 안정적으로 citation하지만, 중간 50K token의 중요한 발견은 research 질문에 직접 답하더라도 자주 놓칩니다. 집계 입력을 어떻게 재구성해야 하나요?
...
- C) 집계 입력 시작 부분에 핵심 발견 summary를 batch하고, 세부 결과를 명시적 섹션 제목으로 구성해 탐색을 쉽게 한다. **[정답]**
...
## 문제 14(Scenario: Multi-Agent Research System)

**상황:** 테스트에서 웹 검색 Agent 출력(페이지 콘텐츠 포함 85K token)과 문서 분석 Agent 출력(사고 과정 포함 70K token)을 합치면 155K token이지만, 종합 Agent는 50K token 미만 입력에서 가장 잘 동작합니다. 가장 효과적인 솔루션은 무엇인가요?
...
**A인 이유:** upstream Agent가 structured data를 반환하도록 수정하면 필수 정보를 보존하면서 token량을 source에서 줄여 근본 원인을 해결합니다. 종합 단계 개선에 기여하지 않는 부피 큰 페이지 콘텐츠와 추론 흔적 전달을 피합니다.

---

## 문제 15(Scenario: Multi-Agent Research System)
...
**D인 이유:** 제한 범위 사실 validation tool은 종합 Agent가 단순 확인의 85%를 직접 처리하게 해 대부분의 루프를 제거하면서, 복잡한 validation 15%에는 Coordinator 위임 경로를 보존합니다. 이는 최소 권한을 적용하면서 지연 시간을 크게 줄입니다.

---

## Scenario: 지속적 통합을 위한 Claude Code

---

## 문제 16(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** CI pipeline이 코드 리뷰를 위한 프로젝트 context를 제공하기 위해 CLAUDE.md를 사용하며, Claude Code CLI를 `--print` 모드로 실행합니다. 개발자들은 대체로 리뷰가 실질적이라고 느낍니다. 하지만 발견 사항을 workflow에 통합하기 어렵다고 보고합니다. Claude가 서술형 문단을 출력해 PR 댓글로 수동 복사해야 하기 때문입니다. 팀은 각 발견 사항을 코드의 관련 위치에 별도 인라인 PR 댓글로 자동 게시하고 싶어 하며, 이를 위해 파일 경로, 줄 번호, severity 수준, 제안 수정이 포함된 structured data가 필요합니다. 가장 효과적인 접근 방식은 무엇인가요?
...
## 문제 17(Scenario: 지속적 통합을 위한 Claude Code)
...
**A인 이유:** 생성자의 추론에 접근하지 않는 두 번째 독립 Claude Code instance는 확증 편향을 피함으로써 근본 원인을 직접 다룹니다. 이 "fresh eyes" 관점은 작성자가 합리화한 이슈를 다른 리뷰어가 잡아내는 사람 동료 리뷰와 유사합니다.

---

## 문제 18(Scenario: 지속적 통합을 위한 Claude Code)
...
- B) 비synchronous 모델은 요청 중간에 tool을 실행하고 Claude가 분석을 계속할 수 있도록 결과를 반환할 수 없다. **[정답]**
- C) Batch API는 요청 매개변수의 tool 정의를 지원하지 않는다.
- D) 최대 24시간의 batch 처리 지연이 풀 리퀘스트 피드백에는 너무 느리지만, 그 외에는 workflow가 작동한다.

**B인 이유:** "fire-and-forget" 비synchronous Batch API 모델에는 요청 중 tool 호출을 가로채고, tool을 실행한 뒤 Claude가 분석을 계속하도록 결과를 반환하는 메커니즘이 없습니다. 이는 단일 논리 상호작용 안에서 여러 tool 요청/응답 라운드가 필요한 반복적 tool 호출 workflow와 근본적으로 맞지 않습니다.

---

## 문제 19(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** CI/CD 시스템은 세 가지 Claude 기반 분석을 실행합니다. (1) 완료 전까지 병합을 차단하는 모든 PR의 빠른 스타일 검사, (2) 전체 코드베이스에 대한 포괄적 주간 보안 감사, (3) 최근 변경 module에 대한 야간 테스트 케이스 생성. Message Batches API는 50% 절감을 제공하지만 처리에 최대 24시간이 걸릴 수 있습니다. 허용 가능한 개발자 경험을 유지하면서 API 비용을 최적화하려고 합니다. 각 작업과 API 접근 방식을 올바르게 매칭한 조합은 무엇인가요?

**올바른 조합은 무엇인가요?**

- A) 50% 절감을 최대화하기 위해 세 작업 모두 Message Batches API를 사용하고, pipeline이 batch 완료를 polling하도록 구성한다.
- B) PR 스타일 검사는 synchronous 호출을 사용하고, 주간 보안 감사와 야간 테스트 생성에는 Message Batches API를 사용한다. **[정답]**
- C) 세 작업 모두 synchronous 호출을 사용해 일관된 응답 시간을 유지하고, 워크로드 전반의 비용 절감은 prompt caching에 의존한다.
- D) PR 스타일 검사와 야간 테스트 생성은 synchronous 호출을 사용하고, 주간 보안 감사에만 Message Batches API를 사용한다.

**B인 이유:** PR 스타일 검사는 개발자를 차단하며 synchronous 호출을 통한 즉각 응답이 필요합니다. 반면 주간 보안 감사와 야간 테스트 생성은 최대 24시간 batch window를 허용할 수 있는 유연한 마감의 예약 작업이므로 둘 다 50% 절감을 얻을 수 있습니다.

---

## 문제 20(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** 자동 리뷰가 실제 이슈를 찾지만 개발자들은 피드백이 실행 가능하지 않다고 보고합니다. 발견 사항에는 정확히 무엇을 바꿔야 하는지 지정하지 않은 채 "복잡한 티켓 routing 로직"이나 "잠재적 null pointer" 같은 문구가 포함됩니다. "항상 구체적 수정 제안을 포함하라" 같은 상세 지시를 추가해도 모델 출력은 여전히 때로는 상세하고 때로는 모호하게 일관되지 않습니다. 어떤 prompting 기법이 일관되게 실행 가능한 피드백을 가장 안정적으로 생성하나요?
...
**D인 이유:** Few-shot 예시는 지시만으로 출력이 가변적일 때 일관된 출력 형식을 달성하는 가장 효과적인 기법입니다. 원하는 구조(이슈, 위치, 구체적 수정)를 보여주는 예시 3-4개는 모델이 따를 구체적 패턴을 제공하며, 추상적 지시보다 더 신뢰할 수 있습니다.

---

## 문제 21(Scenario: 지속적 통합을 위한 Claude Code)
...
**B인 이유:** Deep analysis는 이미 밤새 실행되고 지연을 허용하며 결과 게시 전 polling 모델을 사용하므로 batch 처리에 이상적인 후보입니다. 이는 Message Batches API의 비synchronous, polling 기반 architecture와 일치하면서 50% 절감을 제공합니다.

---

## 문제 22(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** 자동 리뷰가 댓글과 docstring을 분석합니다. 현재 prompt는 Claude에게 "댓글이 정확하고 최신인지 확인하라"고 지시합니다. 발견 사항은 TODO 표시나 단순 설명 같은 허용 가능한 패턴을 자주 문제로 표시하면서, 코드가 더 이상 구현하지 않는 동작을 설명하는 댓글은 놓칩니다. 이 일관되지 않은 분석의 근본 원인을 다루는 변경은 무엇인가요?

**근본 원인을 다루는 변경은 무엇인가요?**
...
**D인 이유:** 명시적 기준, 즉 댓글이 주장하는 동작이 실제 코드 동작과 모순될 때만 표시한다는 기준은 모호한 지시를 문제의 정확한 정의로 대체해 근본 원인을 직접 다룹니다. 이는 허용 가능한 패턴에 대한 false positive과 실제 오해를 부르는 댓글 누락을 줄입니다.

---

## 문제 23(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** 자동 코드 리뷰 시스템이 일관되지 않은 severity 평가를 보입니다. null pointer 위험 같은 유사 이슈가 어떤 PR에서는 "critical"로, 다른 PR에서는 "medium"으로 평가됩니다. 개발자 설문은 신뢰가 떨어지고 있음을 보여줍니다. 많은 개발자가 "절반은 틀렸다"며 발견 사항을 읽지 않고 무시하기 시작합니다. false positive률이 높은 범주는 정확한 범주에 대한 신뢰까지 훼손합니다. 개발자 신뢰를 회복하면서 시스템을 개선하는 가장 좋은 접근은 무엇인가요?

**개발자 신뢰를 가장 잘 회복하는 접근은 무엇인가요?**

- A) false positive률이 높은 범주(스타일, 명명, 문서화)를 일시적으로 비활성화하고, prompt를 개선하는 동안 고정밀 범주만 유지한다. **[정답]**
...
- D) 전체 false positive률을 낮추기 위해 모든 범주에 균일한 엄격도 완화를 적용한다.

**A인 이유:** false positive률이 높은 범주를 일시적으로 비활성화하면 개발자가 모든 것을 무시하게 만드는 노이즈를 제거해 신뢰 하락을 즉시 멈출 수 있습니다. 동시에 보안과 정확성 같은 고정밀 범주의 가치를 보존합니다. 또한 문제 범주를 다시 활성화하기 전에 prompt를 개선할 여지를 만듭니다.

---

## 문제 24(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** 자동 리뷰가 각 PR에 대해 테스트 케이스 제안을 생성합니다. 강의 완료 추적을 추가하는 PR을 리뷰할 때 Claude는 테스트 케이스 10개를 제안했지만, 개발자 피드백에 따르면 6개는 기존 테스트 스위트가 이미 다루는 중복 scenario였습니다. 어떤 변경이 중복 제안을 가장 효과적으로 줄이나요?

**가장 효과적인 변경은 무엇인가요?**

- A) Claude가 이미 다뤄진 scenario를 판단할 수 있도록 기존 테스트 파일을 context에 포함한다. **[정답]**
- B) Claude가 가장 가치 있는 케이스를 먼저 우선순위화한다고 가정하고 제안 요청 수를 10개에서 5개로 줄인다.
- C) 성공 경로가 아니라 엣지 케이스와 오류 조건에만 집중하라는 지시를 추가한다.
- D) 설명이 키워드 겹침으로 기존 테스트 이름과 일치하는 제안을 필터링하는 후처리를 구현한다.

**A인 이유:** 기존 테스트 파일을 포함하면 중복의 근본 원인을 해결합니다. Claude는 어떤 테스트가 이미 존재하는지 알아야만 이미 커버된 scenario 제안을 피할 수 있습니다. 이를 통해 Claude는 실제로 새롭고 가치 있는 테스트를 제안하는 데 필요한 정보를 얻습니다.

---

## 문제 25(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** 초기 자동 리뷰가 12개 발견 사항을 식별한 뒤 개발자가 이슈를 해결하기 위해 새 커밋을 푸시합니다. 리뷰를 다시 실행하면 8개 발견 사항이 나오지만, 개발자들은 새 커밋에서 이미 수정된 코드에 대한 이전 댓글 5개가 중복된다고 보고합니다. 철저함을 유지하면서 이 중복 피드백을 제거하는 가장 효과적인 방법은 무엇인가요?

**중복 피드백을 제거하는 가장 효과적인 방법은 무엇인가요?**

- A) 중간 커밋을 건너뛰고 PR 생성 시점과 최종 병합 전 상태에서만 리뷰를 실행한다.
...
**D인 이유:** 이전 리뷰 발견 사항을 context에 포함하면 Claude가 새 문제와 최근 커밋에서 이미 해결된 문제를 구분할 수 있습니다. 이는 리뷰의 철저함을 유지하면서 Claude의 추론을 사용해 수정된 코드에 대한 중복 피드백을 피합니다.

---

## 문제 26(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** pipeline 스크립트가 `claude "Analyze this pull request for security issues"`를 실행하지만 작업이 무기한 멈춥니다. 로그는 Claude Code가 대화형 입력을 기다리고 있음을 보여줍니다. 자동화 pipeline에서 Claude Code를 실행하는 올바른 접근 방식은 무엇인가요?
...
**B인 이유:** `-p`(또는 `--print`) 플래그는 Claude Code를 비대화형으로 실행하는 문서화된 방법입니다. prompt를 처리하고 결과를 stdout에 출력한 뒤 사용자 입력을 기다리지 않고 종료합니다. CI/CD pipeline에 이상적입니다.

---

## 문제 27(Scenario: 지속적 통합을 위한 Claude Code)
...
**B인 이유:** 집중된 파일별 패스는 주의 분산이라는 근본 원인을 다루어 일관된 깊이와 신뢰할 수 있는 로컬 이슈 탐지를 보장합니다. 이후 별도 통합 지향 패스가 의존성과 데이터 흐름 상호작용 같은 파일 간 문제를 다룹니다.

---

## 문제 28(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** 자동 코드 리뷰는 풀 리퀘스트당 평균 15개 발견 사항을 생성하며, 개발자는 40% false positive률을 보고합니다. 병목은 조사 시간입니다. 개발자는 수정할지 무시할지 결정하기 전에 각 발견 사항을 클릭해 Claude의 근거를 읽어야 합니다. CLAUDE.md에는 이미 허용 가능한 패턴에 대한 포괄적 규칙이 포함되어 있고, 이해관계자들은 개발자가 보기 전에 발견 사항을 필터링하는 모든 접근을 거부했습니다. 조사 시간을 가장 잘 줄이는 변경은 무엇인가요?
...
**A인 이유:** 각 발견 사항에 근거와 confidence를 직접 포함하면 개발자가 각 발견 사항을 열어보지 않고도 빠르게 triage할 수 있어 조사 시간이 줄어듭니다. 모든 발견 사항은 계속 보이므로 "필터링 없음" 제약도 충족하면서 개발자 의사결정을 빠르게 합니다.

---

## 문제 29(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** 자동 코드 리뷰 분석 결과 발견 범주별 false positive률 차이가 큽니다. 보안/정확성 발견은 false positive 8%, 성능 발견은 18%, 스타일/명명 발견은 52%, 문서화 발견은 48%입니다. 개발자 설문은 신뢰가 떨어지고 있음을 보여줍니다. 많은 개발자가 "절반은 틀렸다"며 발견 사항을 읽지 않고 무시하기 시작합니다. false positive률이 높은 범주는 정확한 범주에 대한 신뢰까지 훼손합니다. 개발자 신뢰를 회복하면서 시스템을 개선하는 가장 좋은 접근은 무엇인가요?
...
## 문제 30(Scenario: 지속적 통합을 위한 Claude Code)

**상황:** 팀은 자동 분석 API 비용을 줄이고 싶어 합니다. 현재 synchronous Claude 호출은 두 workflow를 지원합니다. (1) 개발자가 병합하기 전에 반드시 완료되어야 하는 차단형 병합 전 검사, (2) 다음 날 아침 리뷰를 위해 밤새 생성되는 기술 부채 보고서. 관리자는 50% 절감을 위해 둘 다 Message Batches API로 옮기자고 제안합니다. 이 제안을 어떻게 평가해야 하나요?

**이 제안을 어떻게 평가해야 하나요?**

- A) 둘 다 batch 처리로 옮기고, batch가 너무 오래 걸리면 synchronous 호출로 fallback한다.
- B) 두 workflow 모두 batch 처리로 옮기고 완료 확인을 위해 상태를 polling한다.
- C) 기술 부채 보고서에만 batch 처리를 사용하고, 병합 전 검사는 synchronous 호출을 유지한다. **[정답]**
- D) batch 결과 순서 문제를 피하기 위해 두 workflow 모두 synchronous 호출을 유지한다.

**C인 이유:** Message Batches API 처리는 최대 24시간 걸릴 수 있고 지연 시간 SLA가 없어, 야간 기술 부채 보고서에는 허용 가능하지만 개발자가 기다리는 차단형 병합 전 검사에는 허용되지 않습니다. 이는 각 workflow를 지연 요구사항에 맞는 API에 매칭합니다.

---

## Scenario: Claude Code를 사용한 코드 생성

---

## 문제 31(Scenario: Claude Code를 사용한 코드 생성)
...
**B인 이유:** 구체적인 입력-출력 예시는 Claude에게 정확히 기대되는 변환 결과를 보여줌으로써 산문 설명의 모호성을 제거합니다. 이는 필드 중첩과 타임스탬프 형식에 대한 명확한 패턴을 제공해 텍스트 요구사항 오해라는 근본 원인을 직접 다룹니다.

---

## 문제 32(Scenario: Claude Code를 사용한 코드 생성)
...
**B인 이유:** Slack 통합에는 architecture 함의가 크게 다른 여러 유효한 접근이 있고 요구사항도 모호합니다. 계획 모드는 webhook, bot token, Slack App 간 trade-off를 평가하고 구현 전에 접근 방식을 맞출 수 있게 합니다.

---

## 문제 33(Scenario: Claude Code를 사용한 코드 생성)
...
**D인 이유:** CLAUDE.md 내용은 모든 session에 로드되어 코딩 표준과 테스트 규칙이 항상 적용되게 합니다. 반면 Skills는 Claude가 트리거 키워드를 감지할 때 on-demand로 호출되므로 PR 리뷰, 배포, migration 같은 workflow별 지침에 이상적입니다.

---

## 문제 34(Scenario: Claude Code를 사용한 코드 생성)
...
**A인 이유:** 계획 모드는 모놀리스 분리 같은 복잡한 architecture 재구성에 적합한 전략입니다. 많은 파일에 걸친 잠재적으로 비용 큰 변경을 확정하기 전에 안전한 탐색과 경계에 대한 정보 기반 결정을 가능하게 합니다.

---

## 문제 35(Scenario: Claude Code를 사용한 코드 생성)
...
- D) 표시 전에 모든 결과를 짧은 summary으로 압축하라는 지시를 Skill에 추가한다.

**A인 이유:** `context: fork`는 분석을 격리된 subagent context에서 실행해 큰 출력이 메인 session context window를 오염시키지 않게 하고 Claude가 원래 작업을 놓치지 않게 합니다. 전체 분석 기능을 보존하면서 메인 session의 반응성을 유지합니다.

---

## 문제 36(Scenario: Claude Code를 사용한 코드 생성)
...
**C인 이유:** 같은 이름의 개인 Skill은 프로젝트 Skill보다 우선합니다. `~/.claude/skills/commit/SKILL.md`의 개인 Skill은 팀의 프로젝트 Skill을 override해, 개발자가 개인 용도로 익숙한 `/commit` 명령 이름을 유지하면서 workflow를 사용자 지정할 수 있게 합니다. 이 접근은 원래 명령 이름을 보존해 개발자 workflow를 개선하면서 팀원에게 영향을 주지 않으므로 A보다 낫습니다.

---

## 문제 37(Scenario: Claude Code를 사용한 코드 생성)
...
## 문제 38(Scenario: Claude Code를 사용한 코드 생성)
...
**D인 이유:** on-demand로 호출되는 Skill은 새 endpoint를 생성할 때만 예시 context를 로드하고 디버깅이나 리뷰 같은 무관한 작업 중에는 로드하지 않습니다. 이는 메인 context를 깔끔하게 유지하면서 필요할 때 높은 품질의 생성을 보존합니다.

---

## 문제 39(Scenario: Claude Code를 사용한 코드 생성)
...
## 문제 40(Scenario: Claude Code를 사용한 코드 생성)
...
**D인 이유:** `.claude/rules/` 파일과 YAML frontmatter, glob 패턴(예: `**/*.test.tsx`, `src/api/**/*.ts`)은 디렉터리 구조와 관계없이 결정적인 경로 기반 규칙 적용을 가능하게 합니다. 이는 분산된 테스트 파일 같은 교차 패턴에 가장 지원되는 접근입니다.

---

## 문제 41(Scenario: Claude Code를 사용한 코드 생성)
...
**B인 이유:** 프로젝트 저장소 안의 `.claude/commands/` 아래에 사용자 지정 슬래시 명령을 두면 버전 관리되며 저장소를 클론하거나 업데이트하는 모든 개발자가 자동으로 사용할 수 있습니다. 이는 Claude Code에서 프로젝트 수준 사용자 지정 명령의 의도된 위치입니다.

---

## 문제 42(Scenario: Claude Code를 사용한 코드 생성)
...
## 문제 43(Scenario: Claude Code를 사용한 코드 생성)
...
**B인 이유:** `context: fork`는 Skill을 격리된 subagent context에서 실행해 탐색 논의가 메인 대화 기록을 오염시키지 않게 합니다. 이를 통해 거부된 접근과 brainstorming context가 후속 구현 작업에 영향을 주는 것을 방지합니다.

---

## 문제 44(Scenario: Claude Code를 사용한 코드 생성)
...
**C인 이유:** 환경 변수 치환을 사용하는 프로젝트 `.mcp.json`은 관용적입니다. MCP 구성에 대한 단일 버전 관리 진실 공급원을 제공하면서 각 개발자가 환경 변수로 자격 증명을 제공할 수 있게 합니다. 변수를 문서화하면 비밀을 커밋하지 않고 온보딩이 쉬워집니다.

---

## 문제 45(Scenario: Claude Code를 사용한 코드 생성)
...
**구현 일관성을 유지하면서 작업을 완료하는 가장 효과적인 접근 방식은 무엇인가요?**
...
**A인 이유:** Explore subagent는 상세한 발견 출력을 별도 context에 격리하고 메인 대화에는 간결한 summary만 반환합니다. 이는 보존된 context가 가장 중요한 공동 설계와 일관된 구현 단계에 메인 context window를 남깁니다.

---

## Scenario: 고객 지원 Agent

---

## 문제 46(Scenario: 고객 지원 Agent)
...
- D) 다양한 다면적 billing scenario에 대한 이상적인 tool 호출 순서를 보여주는 few-shot 예시를 system prompt에 추가한다.
...
## 문제 49(Scenario: 고객 지원 Agent)

**상황:** Agent의 최초 접촉 해결률은 목표 80%보다 훨씬 낮은 55%입니다. 로그는 Agent가 단순한 케이스(사진 증거가 있는 손상 상품의 표준 교체)는 escalation하면서, 정책 예외가 필요한 복잡한 상황은 자율적으로 처리하려 한다는 것을 보여줍니다. escalation calibration을 개선하는 가장 효과적인 방법은 무엇인가요?

**escalation calibration을 개선하는 가장 효과적인 방법은 무엇인가요?**

- A) 각 응답 전에 Agent가 confidence를 1-10 척도로 자체 평가하게 하고, confidence가 임계값 아래로 떨어지면 자동으로 사람에게 routing한다.
...
**C인 이유:** few-shot 예시가 포함된 명시적 escalation 기준은 단순 케이스와 복잡한 케이스 사이의 불명확한 결정 경계라는 근본 원인을 직접 다룹니다. 추가 인프라 없이 Agent에게 언제 escalation하고 언제 자율 해결할지 가르치는 가장 비례적이고 효과적인 첫 개입입니다.

---

## 문제 50(Scenario: 고객 지원 Agent)

**상황:** `get_customer`와 `lookup_order`를 호출한 후 Agent는 사용 가능한 모든 시스템 데이터를 가지고 있지만 여전히 불확실성에 직면합니다. 어떤 상황이 `escalate_to_human` 호출을 가장 정당화하나요?
...
**C인 이유:** 이는 실제 정책 공백입니다. 회사 규칙은 자사 사이트의 가격 인하는 다루지만 경쟁사 가격 매칭은 다루지 않습니다. Agent는 정책을 지어내면 안 되며 기존 규칙을 어떻게 해석하거나 확장할지 사람의 판단으로 escalation해야 합니다.

---

## 문제 51(Scenario: 고객 지원 Agent)

**상황:** 프로덕션 로그에 따르면 12%의 경우 Agent가 `get_customer`를 건너뛰고 고객이 제공한 이름만으로 `lookup_order`를 직접 호출해, 때로는 계정을 잘못 식별하고 잘못된 환불을 유발합니다. 이 안정성 문제를 가장 효과적으로 해결하는 변경은 무엇인가요?

**가장 효과적인 변경은 무엇인가요?**

- A) 고객이 자발적으로 주문 세부사항을 제공하더라도 Agent가 항상 `get_customer`를 먼저 호출하는 few-shot 예시를 추가한다.
- B) 각 요청을 분석하고 해당 요청 유형에 적절한 tool 일부만 활성화하는 routing classifier를 구현한다.
- C) `get_customer`가 validated 고객 식별자를 반환할 때까지 `lookup_order`와 `process_refund`를 차단하는 프로그래밍적 사전조건을 추가한다. **[정답]**
- D) 주문 작업 전 `get_customer`를 통한 고객 validation이 필수라고 system prompt를 강화한다.

**C인 이유:** 프로그래밍적 사전조건은 필요한 순서가 지켜진다는 결정적 보장을 제공합니다. LLM 동작과 관계없이 validation을 건너뛸 가능성을 제거하므로 가장 효과적인 접근입니다.

---

## 문제 52(Scenario: 고객 지원 Agent)

**상황:** 프로덕션 지표는 복잡한 billing 분쟁이나 다중 주문 반품을 해결할 때, 해결이 기술적으로 맞더라도 고객 만족도 점수가 단순 케이스보다 15% 낮다는 것을 보여줍니다. 근본 원인 분석에 따르면 Agent는 정확한 해결책을 제공하지만 근거 설명이 일관되지 않습니다. 때로는 관련 정책 세부사항을 빠뜨리고, 때로는 일정 정보나 다음 단계를 놓칩니다. 구체적인 context 공백은 케이스마다 다릅니다. 사람 감독을 추가하지 않고 솔루션 품질을 개선하고 싶습니다. 가장 효과적인 접근은 무엇인가요?
...
- C) 정의된 복잡도 지표를 기준으로 복잡한 케이스를 routing해 모델을 Haiku에서 Sonnet으로 업그레이드한다.
- D) 정책 context, 일정, 다음 단계를 포함하는 방법을 보여주는 다섯 가지 일반 복잡 케이스에 대한 완전한 설명 few-shot 예시를 system prompt에 구현한다.

**A인 이유:** 자기 비판 단계(evaluator-optimizer 패턴)는 정책 context, 일정, 다음 단계 같은 구체 기준에 대해 Agent가 자신의 초안을 평가하도록 강제해 설명 완전성의 불일치를 직접 다룹니다. 이는 사람 감독 없이 케이스별 공백을 잡아냅니다.

---

## 문제 53(Scenario: 고객 지원 Agent)

**상황:** 프로덕션 지표는 Agent가 해결당 평균 4회 이상의 API 루프를 사용함을 보여줍니다. 분석 결과 Claude는 처음부터 둘 다 필요한 경우에도 `get_customer`와 `lookup_order`를 별도 순차 턴으로 요청하는 경우가 많습니다. 루프 수를 줄이는 가장 효과적인 방법은 무엇인가요?

**루프를 줄이는 가장 효과적인 방법은 무엇인가요?**

- A) 요청된 tool과 병렬로 필요할 가능성이 있는 tool을 자동 호출하고 요청 여부와 관계없이 모든 결과를 반환하는 speculative execution을 구현한다.
...
**D인 이유:** Claude에게 관련 tool 요청을 단일 턴에 묶도록 prompt하면 한 번에 여러 tool을 요청하는 Claude의 기본 능력을 활용할 수 있습니다. 이는 최소한의 architecture 변경으로 순차 호출 패턴을 직접 고칩니다.

---

## 문제 54(Scenario: 고객 지원 Agent)

**상황:** 프로덕션 로그는 고객이 특정 금액을 언급하지만(예: "제가 말한 15% 할인") Agent가 잘못된 값으로 응답하는 패턴을 보여줍니다. 조사 결과 이러한 세부사항은 20턴 이상 전에 언급되었고 "프로모션 가격이 논의됨" 같은 모호한 summary으로 압축되었습니다. 가장 효과적인 수정은 무엇인가요?
...
- A) 85% 이상 confidence에서는 자율적으로 행동하고 그 아래에서는 clarification를 요청하는 confidence 점수 시스템을 구현한다.
...
## 문제 56(Scenario: 고객 지원 Agent)

**상황:** 프로덕션 로그는 일관된 패턴을 보여줍니다. 고객이 메시지에 "account"라는 단어를 포함하면(예: "I want to check my account for an order I made yesterday") Agent가 78%의 경우 `get_customer`를 먼저 호출합니다. 고객이 유사한 요청을 "account" 없이 표현하면(예: "I want to check an order I made yesterday") 93%의 경우 `lookup_order`를 먼저 호출합니다. tool 설명은 명확하고 모호하지 않습니다. 이 불일치의 가장 가능성 높은 근본 원인은 무엇인가요?
...
**D인 이유:** 입력 형식, 예시 쿼리, 엣지 케이스, 명확한 경계를 포함해 tool 설명을 확장하면 LLM이 유사한 tool을 구분하기에 충분한 정보를 받지 못하는 최소 설명이라는 근본 원인을 직접 고칩니다. 이는 LLM이 tool 선택에 사용하는 기본 메커니즘을 개선하는 저노력, 고효과의 첫 단계입니다.

---

## 문제 58(Scenario: 고객 지원 Agent)

**상황:** 지원 Agent를 위한 Agent 루프를 구현하고 있습니다. 각 Claude API 호출 후 루프를 계속할지(요청된 tool을 실행하고 Claude를 다시 호출) 또는 중지할지(최종 답변을 고객에게 제시) 결정해야 합니다. 이 결정을 무엇이 좌우하나요?
...
- B) tool별로 예시를 그룹화한다. 모든 `get_customer` scenario를 모은 뒤 모든 `lookup_order` scenario를 모은다.
- C) 모호한 scenario를 겨냥한 4-6개 예시를 추가하고, 각 예시에 왜 한 tool이 가능한 대안보다 선택되었는지 근거를 포함한다. **[정답]**
- D) 각 tool의 일반적 scenario에 대한 올바른 tool 선택을 보여주는 명확하고 모호하지 않은 요청 예시 10-15개를 추가한다.

**C인 이유:** 오류가 발생하는 특정 모호한 scenario를 겨냥하고, 왜 한 tool이 대안보다 나은지 명시적 근거를 포함한 few-shot 예시는 엣지 케이스에 필요한 비교 판단 과정을 모델에 가르칩니다. 이는 일반 예시나 선언적 규칙보다 효과적입니다.

---

## 문제 61(Scenario: Conversational AI Architecture Patterns)
...
**D인 이유:** 두 tool과 token-binding 접근은 사전 미리보기 없이 실행하는 것을 architecture적으로 불가능하게 만듭니다. 실행 tool은 미리보기 tool만 생성할 수 있는 token을 문자 그대로 요구합니다. 이는 LLM의 지시 준수(C), timing heuristic(A), orchestration 인프라(B)에 의존하지 않고 코드 수준에서 제약을 강제하는 유일한 접근입니다.

---

## 문제 62(Scenario: Conversational AI Architecture Patterns)
...
## 문제 63(Scenario: Conversational AI Architecture Patterns)
...
- B) 두 scenario에 대해 별도 추천을 제공한다.
- C) 가장 최근에 말한 선호로 진행한다.
- D) 충돌을 다루지 않고 균형 잡힌 포트폴리오를 추천한다.

**A인 이유:** 사용자 선호가 직접 모순될 때 충돌을 드러내고 clarification를 요청하는 것이 추천이 사용자의 진짜 의도에 맞도록 보장하는 유일한 방법입니다. 다른 접근은 틀릴 수 있는 가정을 포함합니다. 수익 극대화와 낮은 위험 허용도는 근본적으로 양립하기 어려운 목표이며 사람의 결정이 필요합니다.

---

## 문제 64(Scenario: Conversational AI Architecture Patterns)
...
## 문제 65(Scenario: Conversational AI Architecture Patterns)
...
- C) 중요한 structured data(알레르기, 수량, 선호)를 extraction하고, 일반 논의는 summary하며, 최근 교환은 원문 그대로 유지한다. **[정답]**
- D) 전체 대화를 외부에 저장하고 semantic search로 관련 부분을 검색한다.

**C인 이유:** 하이브리드 접근은 가장 높은 가치의 정보를 가장 낮은 비용으로 보존합니다. 알레르기와 레시피 수량 같은 중요 사실은 압축된 structured 블록으로 extraction해 summary 중 발생하는 정밀도 손실을 방지하고, 일반 논의는 summary하며, 최근 교환은 대화 일관성을 위해 원문 그대로 유지합니다. A와 B는 중요한 식이 정보를 잃을 위험이 있고, D는 단일 요리 session에는 architecture적으로 과합니다.

---

## 문제 66(Scenario: Conversational AI Architecture Patterns)
...
**A인 이유:** 하이브리드 접근은 문제의 두 차원을 모두 다룹니다. 대화 일관성에 중요한 정확한 최근 context를 유지하면서, 쌍이 삭제될 때 전체 손실을 막기 위해 이전 선호의 압축 표현을 유지합니다. window 확대(C)는 같은 문제를 지연할 뿐입니다. Vector search(B)는 현재 쿼리와 의미적으로 유사하지 않은 중요한 context를 놓칠 수 있습니다. 매 턴 전체 summary(D)은 overhead를 추가하고 summary 오류를 누적합니다.

---

## 문제 67(Scenario: Conversational AI Architecture Patterns)
...
## 문제 68(Scenario: Conversational AI Architecture Patterns)
...
## 문제 69(Scenario: Conversational AI Architecture Patterns)
...
## 문제 70(Scenario: Conversational AI Architecture Patterns)
...
**B인 이유:** 선언적 규칙이 있는 2,800token system prompt는 모델이 매 턴 이를 추론해야 하므로 drift에 취약합니다. 장황한 규칙을 올바른 숙련도 수준 적응을 보여주는 구체적인 few-shot 예시로 바꾸면 모델이 맞출 명확한 행동 패턴을 제공합니다. 이는 여러 턴에 걸쳐 추상 지시보다 더 안정적으로 따릅니다. Reminder 주입(A)은 도움이 되지만 증상을 다루고, 끝 batch(C)는 초기에는 도움이 되지만 턴 수준 drift에는 부족하며, 재생성(D)은 비용이 큰 교정입니다.

---

## 문제 71(Scenario: Conversational AI Architecture Patterns)
...
**B인 이유:** system prompt는 전체 대화에 적용되는 지속적 행동 제약과 지침을 위해 특별히 설계되었습니다. 각 사용자 메시지에 붙이는 것(A)은 중복 overhead입니다. 첫 번째 assistant 메시지(C)는 모델이 자신의 이전 진술에서 벗어날 수 있어 신뢰할 수 없습니다. 환경 변수(D)는 모델 동작에 영향을 주지 않습니다.

---

## 문제 72(Scenario: Conversational AI Architecture Patterns)
...
## 문제 73(Scenario: Conversational AI Architecture Patterns)
...
**D인 이유:** 다음 사용자 메시지에 상태 업데이트를 prefix로 주입하면 대화 흐름을 방해하지 않고 자연스러운 대화 경계에서 실시간 context를 주입할 수 있습니다. system prompt 수정(A)은 session 재구성이 필요하거나 architecture적으로 번거롭습니다. 합성 사용자 메시지(B)는 자연스러운 대화 흐름을 깨고 attribution을 혼란스럽게 할 수 있습니다. 매 턴 tool 호출 강제(C)는 이벤트가 드문 경우 낭비입니다.

---

## 문제 74(Scenario: Conversational AI Architecture Patterns)
...
## 문제 75(Scenario: Conversational AI Architecture Patterns)
...
## 문제 76(Scenario: Conversational AI Architecture Patterns)
...
5. 사람에게 routing: 필드 수준 confidence 점수, 문서 유형 분석
...
5. **Prompt Engineering을 연습하세요** — 모호한 scenario를 위한 few-shot 예시, 명시적 리뷰 기준, 큰 코드 리뷰를 위한 다중 패스 architecture를 추가합니다.
...
8. **실제 시험 전에 연습 시험을 보세요.** 같은 scenario와 형식을 사용합니다.
